...
class InfoView(private val mv: MainView): View("Information") {
    ...
    // TODO leaflet doesn't work with jfx 21: https://stackoverflow.com/questions/77764181/webview-in-javafx-not-showing-leaflet-map-correct
    // openlayers 9 also doesn't, but 4 !!
    private fun mapHTML(geo: GeoLocation) = """
        <!DOCTYPE html>
        <html>
          <head>
          ...
          <body>
            ...
            <script>
                ...
                var map = new ol.Map({
                  ...
                  layers: [ baseMapLayer],
                  view: new ol.View({
                          center: ol.proj.fromLonLat([${geo.longitude}, ${geo.latitude}]), 
                          zoom: 13
                        })
                });
                ...
                var marker = new ol.Feature({
                  geometry: new ol.geom.Point(
                    ol.proj.fromLonLat([${geo.longitude}, ${geo.latitude}])
                  ),
                });
                ...
                mv.il.currentImage.geo?.also { geo ->
                    field("Location") {
                        webview {
                            ...
                            prefHeight = 270.0
                            runLater {
                                engine.loadContent(mapHTML(geo))
                            }
                        }
                    }
                    field("Links") {
